...
class AboutTheBook(object):
    ...
    def title(self, title):
        return lf().basic_format('title',title)

    def author_name(self, author):
        return lf().basic_format('author',author)

    def edition(self, edition):
        return lf().new_keyword_definition('edition', edition)

    def get_all_values(self, story):
        values = self.title(story.Title), self.author_name(story.Author), self.edition(story.Edition)
        return values
